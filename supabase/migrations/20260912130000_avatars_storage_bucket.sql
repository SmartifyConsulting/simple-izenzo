...
-- belong to under orgs/<org_id>/.... Each policy is dropped first so this can be re-run safely if
-- some of it (e.g. the bucket, or one policy) was already created by hand.
...
drop policy if exists "Org members can upload their org's avatar" on storage.objects;
...
drop policy if exists "Org members can update their org's avatar" on storage.objects;

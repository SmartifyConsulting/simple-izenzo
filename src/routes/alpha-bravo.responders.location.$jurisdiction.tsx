...
export const Route = createFileRoute("/alpha-bravo/responders/location/$jurisdiction")({
  head: () => ({
    meta: [{ title: "Responders by Location — Izenzo Alpha-Bravo" }],
  }),
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-20 sm:py-24">
      ...
      <p className="mt-8 text-[11px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">
        Responders · {jurisdiction ?? jurisdictionSlug}
      </p>
      <h1 className="mt-4 max-w-2xl text-4xl tracking-tight text-foreground sm:text-5xl">
        Verified Responders in {jurisdiction ?? jurisdictionSlug}.
      </h1>
      <p className="mt-6 max-w-2xl leading-relaxed text-muted-foreground">
        Responders that have cleared Izenzo's compliance screening in this location — live from
        our database.
      </p>

      {!isLoading && !jurisdiction && (
        <p className="mt-10 text-sm text-muted-foreground">
          No Responders on file yet for this location.
        </p>
      )}

...
export function guessSideFromWording(text: string): "bid" | "offer" | null {
  ...
  const selling = /\b(offer to sell|quotation|price list|available (for|to) (sale|supply)|ex-?stock|for sale|we sell|i sell|want to sell|selling|we supply|we offer|offering|we export|looking for buyers?|find (a )?buyers?|buyers?|off-?takers?)\b/.test(t);
  if (/request for (proposals?|quotations?|tenders?)/.test(t)) return "bid";
